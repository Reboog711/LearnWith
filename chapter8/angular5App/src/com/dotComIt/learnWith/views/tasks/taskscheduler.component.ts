...
// lower casing this because of static injector error
...
export class TaskScheduler  implements OnInit {
    ...
    onTaskListSchedule() {
        let localDate : Date = new Date(this.schedulerDate.month + '/' + this.schedulerDate.day + '/' + this.schedulerDate.year);
        this.taskService.scheduleTaskList(this.taskModel.scheduledTasks, localDate)
            .subscribe(
                result => {
                    if (result.error) {
