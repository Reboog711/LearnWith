                ...
                setTimeout(() => {
                    ...
                    result.resultObject = this.tasks.filter(
                        function(element, index, array){
                            ...
                            if (!(typeof filter.taskCategoryID === 'undefined') && (filter.taskCategoryID !== null)) {
...
//                            console.log('check start date');
                            ...
                            if (filter.scheduledStartDate) {
                                ...
                                if (filter.scheduledStartDate > element.dateScheduled) {
                                    if ( (filter.scheduledStartDate.getDate() !== element.dateScheduled.getDate()) ||
                                        (filter.scheduledStartDate.getMonth() !== element.dateScheduled.getMonth()) ||
                                        (filter.scheduledStartDate.getFullYear() !== element.dateScheduled.getFullYear()) ) {
                                        return false;
                                    }
                                }
                            }
                            ...
                            if (filter.scheduledEqualDate) {
                                ...
                                if ( (filter.scheduledEqualDate.getDate() !== element.dateScheduled.getDate()) ||
                                    (filter.scheduledEqualDate.getMonth() !== element.dateScheduled.getMonth()) ||
                                    (filter.scheduledEqualDate.getFullYear() !== element.dateScheduled.getFullYear()) ) {
                                    return false;
                                }
                            }


                            // it didn't meet any criteria for being dumped; so leave it in!
                            return true;
        ...
        let o: Observable<ResultObjectVO> = Observable.create(
            (observer: Observer<ResultObjectVO>) => {
                setTimeout(() => {
                    ...
                    result.error = false;
//                reject(result);
                    console.log('task service schedule task');
                    console.log(result);
                    observer.next(result);
                    observer.complete();
                }, 1000);
        ...
        let o: Observable<ResultObjectVO> = Observable.create(
            (observer: Observer<ResultObjectVO>) => {
                setTimeout(() => {
                    ...
                    result.error = false;
                    //                reject(result);
                    console.log('task service complete task');
                    observer.next(result);
                    observer.complete();
                }, 1000);
            });
