...
public class TaskWrapper {
    ...
    @PUT
    @Path("/{taskID}/completed/{completed}")
    @Produces(MediaType.APPLICATION_JSON)
    public TaskVO completeTask(@PathParam("taskID") int taskID, @PathParam("completed") boolean completed){
        TaskVO results = ts.completeTask(taskID, completed);
        if (results == null) {
            RestExceptionVO restException = new RestExceptionVO();
            restException.setMessage("Error Completing Task");
            throw new RestException(restException, Response.Status.INTERNAL_SERVER_ERROR);
        }
        return results;
    }
