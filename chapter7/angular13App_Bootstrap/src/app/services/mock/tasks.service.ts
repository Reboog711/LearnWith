...
export class TasksServiceMock {
          ...
          let result : TaskVO[] = this.dataStoreService.tasks.filter(
            (element, index, array) => {
              ...
              if (filter.endDateAsUTCString) {
                if (new Date(filter.endDateAsUTCString) <
                  new Date(element.dateCreatedAsUTCString)) {
                  return false;
                }
              }
              if (filter.scheduledStartDateAsUTCString) {
                ...
                if (new Date(filter.scheduledStartDateAsUTCString) >
                  new Date(element.dateScheduledAsUTCString)) {
                  return false;
                }
              }
              if (filter.scheduledEndDateAsUTCString) {
                ...
                if (new Date(filter.scheduledEndDateAsUTCString) <
                  new Date(element.dateScheduledAsUTCString)) {
                  return false;
                }
              }
              if (filter.scheduledEqualDateAsUTCString) {
                ...
                let scheduledEqualDatePlusOne =
                  new Date(filter.scheduledEqualDateAsUTCString);
                scheduledEqualDatePlusOne.setDate(scheduledEqualDatePlusOne.getDate()+1);
                if ( !(( scheduledEqualDate <= dateScheduled) &&
                  (scheduledEqualDatePlusOne >= dateScheduled))) {
                  return false;
                }
              }
  ...
  scheduleTaskList(tasks :TaskVO[], schedulerDateAsUTCString?:string):
    Observable<any> {
