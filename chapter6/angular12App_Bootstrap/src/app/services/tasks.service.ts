...
export class TasksService {
  ...
  loadTasks(taskFilter:TaskFilterVO) : Observable<TaskVO[]> {
    ...
    if (taskFilter.scheduledStartDateAsUTCString) {
      url.searchParams.append('scheduledStartDateAsUTCString', taskFilter.scheduledStartDateAsUTCString)
    }
    if (taskFilter.scheduledEndDateAsUTCString) {
      url.searchParams.append('scheduledEndDateAsUTCString', taskFilter.scheduledEndDateAsUTCString)
    }
    if (taskFilter.scheduledEqualDateAsUTCString) {
      url.searchParams.append('scheduledEqualDateAsUTCString', taskFilter.scheduledEqualDateAsUTCString)
    }
    if (taskFilter.taskCategoryID) {
      url.searchParams.append('taskCategoryID', taskFilter.taskCategoryID.toString())
    }
  ...
  scheduleTaskList(tasks :TaskVO[], schedulerDateAsUTCString? : string): Observable<any> {
    ...
    taskIDList = taskIDList.slice(0, -1); // remove last character
