...
export class TaskSchedulerComponent {
          ...
          for (let scheduledTaskIndex : number = 0;
            ...
            for (let masterTaskIndex :number = 0;
              ...
              if (this.taskModel.tasks[masterTaskIndex].taskID ===
                ...
                this.taskModel.tasks[masterTaskIndex].dateScheduledAsUTCString =
                  schedulerDateAsUTCString as string;
                break;
              }
            }
          }
  ...
  onTaskListSchedule() {
    debugger;
    const localDate : Date = new Date(this.schedulerDate.year,
      this.schedulerDate.month-1,
      this.schedulerDate.day)
//      localDate.setUTCHours(0,0,0,0)
    const localDateAsString = localDate.toISOString();
    this.scheduleTasks(this.taskModel.scheduledTasks, localDateAsString).subscribe({
      next: (result: any) => {
        this.taskModel.addedTasks = [];
      },
      error: (error: HttpErrorResponse) => {
