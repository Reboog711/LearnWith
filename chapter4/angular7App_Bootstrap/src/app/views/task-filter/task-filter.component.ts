...
export class TaskFilterComponent implements OnInit {
  ...
  filter():void {
    ...
    if (this.startDate) {
      taskFilter.startDate = new Date(this.startDate.year, this.startDate.month-1, this.startDate.day) ;
    } else {
      taskFilter.startDate = null;
    }
    if (this.endDate) {
      taskFilter.endDate = new Date(this.endDate.year, this.endDate.month-1, this.endDate.day) ;
    } else {
      taskFilter.endDate = null;
    }
    if (this.scheduledStartDate) {
      taskFilter.scheduledStartDate = new Date(this.scheduledStartDate.year, this.scheduledStartDate.month-1, this.scheduledStartDate.day) ;
    } else {
      taskFilter.scheduledStartDate = null;
    }
    if (this.scheduledEndDate) {
      taskFilter.scheduledEndDate = new Date(this.scheduledEndDate.year, this.scheduledEndDate.month-1, this.scheduledEndDate.day) ;
    } else {
